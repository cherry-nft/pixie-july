...
const FamilyButton: React.FC<FamilyButtonProps> = ({ children }) => {
  ...
  return (
    <div
      className={cn(
        'rounded-lg border border-gray-200 shadow-md dark:border-gray-700',
        'bg-gradient-to-b from-gray-100 to-gray-200',
        isExpanded
          ? 'w-[204px] bg-gradient-to-b dark:from-gray-700 dark:to-gray-800'
          : 'dark:from-gray-800 dark:to-gray-900 bg-gradient-to-b'
      )}
    >
      <div className="rounded-lg border border-gray-200">
        <div className="rounded-lg border dark:border-gray-600 border-gray-300">
          <div className="rounded-lg border border-gray-400 flex items-center justify-center">
...
const FamilyButtonContainer: FC<FamilyButtonContainerProps> = ({
  ...
  return (
    <motion.div
      className={cn(
        'relative border-gray-300 border shadow-lg flex flex-col space-y-1 items-center text-gray-800 cursor-pointer z-10',
        !isExpanded
    ...
    >
      ...
      >
        {isExpanded ? (
          <motion.div
            className="p-[10px] group bg-gray-700/50 dark:bg-black/50 border border-gray-200/30 hover:border-gray-300 text-gray-50 rounded-full shadow-xl transition-colors duration-300 "
          ...
          >
            <ViewNoneIcon
              className={cn(
                'h-7 w-7 text-gray-200/30 dark:text-gray-400/80 group-hover:text-gray-500 transition-colors duration-200 '
              )}
            />
          </motion.div>
        ) : (
          <motion.div
            className={cn(
              'p-[10px] group bg-gray-200 dark:bg-gray-500/90 text-gray-50 border border-gray-200/10  shadow-xl transition-colors duration-200'
            )}
          ...
          >
            <PlusIcon className="h-7 w-7 text-black dark:text-gray-900" />
          </motion.div>
        )}
      </motion.div>
    </motion.div>
...
export function OgImageSection() {
  ...
  return (
    <div className="flex flex-col items-center pt-4 ">
      <div className="flex space-x-1 border border-none rounded-lg cursor-pointer bg-gray-800  px-[3px] py-[3.2px] shadow-inner">
        {tabs.map((tab, i) => (
          <button
            key={`${tab.id}-i-${i}`}
            onClick={() => handleTabClick(tab.id)}
            className={`${
              activeTab === tab.id ? 'text-white ' : 'hover:text-gray-300/60'
            } relative rounded-lg px-3 py-1.5 text-xs sm:text-sm font-medium text-gray-600  transition focus-visible:outline-1 focus-visible:ring-1 focus-visible:ring-blue-light focus-visible:outline-none`}
            style={{ WebkitTapHighlightColor: 'transparent' }}
          >
            {activeTab === tab.id && (
              <motion.span
                layoutId="family-bubble"
                className="absolute inset-0 z-10 bg-gray-900 mix-blend-difference shadow-inner"
